...
const pvSendValidResponse = (pRes, pResponseObject = {}, pRedirectURL = null) => {
    ...
    //Data
    if (pResponseObject.data) {
        xResponse.data = pResponseObject.data;
    }
    //Metadata
    if (pResponseObject.metadata) {
...
const pvConfigClauses = req => {
    ...
    if (req.query && req.originalUrl && !req.clauses) {
        //Parametros de Busca
        let { page = null, offset = 0, size = null, sort = null, ...xQuery } = req.query;
        req.clauses = {
            limit: { offset },
            sort: req.query.sort
        };
        ...
        if (isNumber(size) && size > 1) {
            req.clauses.limit.size = size;
            page = toNumber(page);
            req.clauses.limit.page = isNumber(page) ? page : 0;
        }
        req.query = xQuery;
...
const pvConfigPages = (pReq, pPages) => {
    ...
    let xPagesCount = div(pPages.total_items, xLimit.size);
...
const pvBuildUrl = (pReq, pPage, pSize) => {
