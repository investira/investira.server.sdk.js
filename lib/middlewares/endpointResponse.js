        ...
        pFunctionToExecute(
            ...
            pRejectedData => {
                ...
                if (
                    ...
                    pRejectedData.message.isBasicMessage ||
...
const pvSendValidResponse = (pRes, pResponseObject = {}, pRedirectURL = null) => {
    ...
    if (!isEmpty(pResponseObject.message)) {
        ...
        if (
            pResponseObject.message.isBasicMessage ||
            ...
            if (
                pResponseObject.message.isBasicMessageSuccess ||
                pResponseObject.message instanceof BasicMessageSuccess
            ) {
...
const pvSendInvalidResponse = (res, pErr) => {
    // @ts-ignore
    gLog.warn(`Please, try to correct this error before it reaches this point!`);
    // @ts-ignore
    gLog.error(pErr);
...
const pvConfigQuery = req => {
    if (req.query && !req.clauses) {
        //Parametros de Busca
        let { page = 0, offset = 0, size, sort, ...xQuery } = req.query;
        req.clauses = {
