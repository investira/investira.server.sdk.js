...
const pvConfigClauses = req => {
    if (req.query && req.originalUrl && !req.clauses) {
        ...
        req.query = xQuery;
        //Pegar somente o endpoint
        req.routeUrl = req.originalUrl.split('?')[0];
    }
};
...
const pvConfigPages = (pReq, pPages) => {
    ...
    const xQuery = pReq.query;
    const xRouteUrl = pReq.routeUrl;
    ...
    if (xLimit && xLimit.page && xPagesCount > 1) {
        let xQueryUrl = '';
        if (xQuery) {
            for (const xKey in xQuery) {
                if (isString(xQuery[xKey])) {
                    xQueryUrl += `&${xKey}=${xQuery[xKey]}`;
                } else {
                    gLog.emerg(`Query esta sendo modificada indevidamente por uma rota ou serviço: ${xRouteUrl}`);
                }
            }
        }
        ...
        if (xLimit.page && xLimit.page > 1) {
            ...
            xPageLinks.first = `${xRouteUrl}?page=1${xSize}${xQueryUrl}`;
            //Paginação - Prev
            xPage = sub(toNumber(xLimit.page), 1);
            xPageLinks.prev = `${xRouteUrl}?page=${xPage}${xSize}${xQueryUrl}`;
        }
        if (xLimit.page && xLimit.page < xPagesCount && xLimit.page >= 1) {
            //Paginação - Next
            xPage = add(toNumber(xLimit.page), 1);
            xPageLinks.next = `${xRouteUrl}?page=${xPage}${xSize}${xQueryUrl}`;
            //Paginação - Last
            xPageLinks.last = `${xRouteUrl}?page=${xPagesCount}${xSize}${xQueryUrl}`;
        }
    }
    //Total de Páginas
    return {
