...
const requestContextMiddleware = (pDBServer, pDOSOptions = { limit: 0 }) => {
    ...
    return async (req, res, next) => {
        ...
        //Resolve e salva ip original do request
        res.locals.remoteAddress =
            req.headers['x-forwarded-for'] || req.connection.remoteAddress;
        gLog.note(req.socket.address().address);
...
const pvDOSMonitor = async (req, res) => {
    return await new Promise(async (pResolve, pReject) => {
        ...
        if (!requestContextState.hosts[res.locals.remoteAddress]) {
            requestContextState.hosts[res.locals.remoteAddress] = { count: 1 };
        } else {
