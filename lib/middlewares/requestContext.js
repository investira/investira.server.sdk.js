 ...
 */
const requestContextMiddleware = (pDBServer, pDOSOptions = { limit: 0 }) => {
    requestContextState.dbServer = pDBServer;
    requestContextState.DOSOptions = pDOSOptions;
    return async (req, res, next) => {
        ...
        gLog.note(req.headers['x-forwarded-for']);
        gLog.note(req.connection.remoteAddress);
        gLog.note(
            ...
            await pvCloseConnection(xCN).then(() => {
                ...
                gLog.note(
                    `end  \t${req.method}:\t${req.originalUrl} ${
                        ...
                        requestContextState.hosts[req.client.remoteAddress]
                            .count
                    }`
                );
            });
...
const pvDOSMonitor = async req => {
    return await new Promise(async (pResolve, pReject) => {
        //Cria contador do host, se não existir
        if (!requestContextState.hosts[req.client.remoteAddress]) {
            requestContextState.hosts[req.client.remoteAddress] = { count: 1 };
        } else {
            //Adiciona 1 ao contador
            requestContextState.hosts[req.client.remoteAddress].count++;
            //Verica se contador é superior ao limite permitido por host
            if (
                requestContextState.DOSOptions.limit > 0 &&
                requestContextState.hosts[req.client.remoteAddress].count >
                    requestContextState.DOSOptions.limit
            ) {
                return pReject(
                    new TooManyRequestsError(
                        `Too Many Requests For [${req.client.remoteAddress}][${
                            req.ip
                        }] ${
                            requestContextState.hosts[req.client.remoteAddress]
                                .count
