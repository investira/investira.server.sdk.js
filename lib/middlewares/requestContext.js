...
function requestContext(pDBServer, pDDOSOptions = { limit: 0 }) {
    ...
    this.set = (pKey, pValue) => {
        const xContextData = pvGetContextData();
        if (xContextData) {
        ...
        if (!xContext) {
            ...
            gLog.error(
                new InternalServerError(
                    `[${xAsyncId}] requestContext inexistente. Verifique se request foi finalizado por timeout, cancelado, e/ou inclua o requestContextMiddleware no express.use.`
                )
            );
            return;
        }
            ...
            pvDDOSMonitor(req, res)
                ...
                .then(() => {
                    ...
                    onFinished(res, async err => {
                        console.log(`onfinished ${res.req.aborted}`);
                        // @ts-ignore
