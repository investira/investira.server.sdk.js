...
function requestContext(pDBServer, pDDOSOptions = { limit: 0 }) {
    ...
    this.getCount = () => {
        return Object.freeze(state.count);
    };
    ...
    this.middleware = function() {
        return (req, res, next) => {
            ...
            gLog.note(`begin\t${req.method}:${req.originalUrl}\t${res.locals.remoteAddress}`);
            ...
            pvDDOSMonitor(req, res)
                ...
                .then(() => {
                    ...
                    onFinished(res, err => {
                        gLog.debug(`onFinished. Aborted=${res.req.aborted}`);
                        ...
                        pvCloseConnection(xCN).then(() => {
                            //Log
                            gLog.info(
                                `Requests Total:${state.count.total} \tRequests Active:${
                                    state.count.active
                                } \tActive Contexts:${this.childrenCount()} \t(Current Context Id:${this.getId()})`
                            );
                            gLog.info(
                                `Memory: ${friendlyByte(process.memoryUsage().rss, 1)} \tHeap Total: ${friendlyByte(
                            ...
                            gLog.note(
                                `end  \t${req.method}:${req.originalUrl}\t${res.locals.remoteAddress} ${state.hosts[res.locals.remoteAddress].count}`
                            );
                        });
                    });
                    //Continua
