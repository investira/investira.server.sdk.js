...
function requestContext(pDBServer, pDDOSOptions = { limit: 0 }) {
    ...
    const xSelf = this; // Para garantir que 'this' se refira ao objeto correto
        ...
        if (!xContext) {
            ...
            gLog.error(
                new InternalServerError(
                    `[${xAsyncId}] requestContext inexistente. ${pKey ? '[key:' + pKey + ']' : ''
     ...
     */
    this.hasContext = () => {
        return !isNull(state.context[xSelf.getId()]);
    };
    /**
    ...
    this.create = () => {
        ...
        state.hook = asyncHooks.createHook({
            ...
            promiseResolve: pAsyncId => {
                ...
                setImmediate(pvDeleteContext.bind(xSelf, pAsyncId));
            }
        });
    ...
    this.middleware = function () {
        return (req, res, next) => {
            res.locals.httpAsyncId = xSelf.getId();
            ...
            try {
                xSelf.set('reqId', state.count.total);
                ...
                //Configura flags de log e conexão no request context
                //para que possam ser utilizados durante todo o request
                xSelf.set('disable_log', req.___disableLog || false);
                delete req.___disableLog;
                xSelf.set('disable_connection', req.___disableConnection || false);
                delete req.___disableConnection;
                ...
                pvDDOSMonitor(req, res)
                    ...
                    .then(() => {
                        req.on('aborted', () => {
                            // @ts-ignore
                            gLog.child({ ip: res?.locals?.remoteAddress }).note(
                                `aborted  \t${req.method}:${req.originalUrl}\t${res?.locals?.remoteAddress} ${state.hosts[res?.locals?.remoteAddress].count
                                }\n`
                            );
                        });
    ...
    /**
 * Recupera se log está desabilitado
 *
 * @returns {boolean}
 */
    const pvIsConnectionEnabled = () => {
        // @ts-ignore
        const xDisableConnection = xSelf.get('disable_connection') || false;
        return !xDisableConnection;
    };
                ...
                if (
                    ...
                    return pReject(
                        new TooManyRequestsError(
                            `Too Many Requests For [${res?.locals?.remoteAddress}][${req.ip}] ${state.hosts[res?.locals?.remoteAddress].count
                            }`
                        )
                    );
                }
    ...
    const pvOpenConnection = res => {
        if (isNull(state.dbServer) || res.req.aborted || !pvIsConnectionEnabled()) {
    ...
    const pvCloseConnection = pCN => {
        ...
        if (isNull(pCN) || !pvIsConnectionEnabled()) {
            return Promise.resolve();
        } else {
