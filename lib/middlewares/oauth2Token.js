...
const { InvalidData } = require('investira.sdk').messages.DataErrors;
const { CreatedMessage } = require('investira.sdk').messages.Success;
...
const oauth2Token = pSource => {
    ...
    const getToken = req => {
        ...
        // @ts-ignore
        gLog.verbose(`oauth2Token middleware ${xType}`);
        ...
        if (xType == 'client_credentials') {
            ...
            return abstract.password(req.user, req.body.username, req.body.password, req.body.scope || null);
        } else if (xType == 'refresh_token') {
            return abstract.refreshToken(req.user, req.body.refresh_token, req.body.scope || null);
    ...
    let abstract = Object.assign(
        {
            ...
            clientCredentials(pClient, pScope) {
                // @ts-ignore
                gLog.error(
                    new InternalServerError('oauth2Token middleware abstract clientCredentials not implemented')
                );
            ...
            password(pClient, pUsername, pPassword, pScope) {
                // @ts-ignore
                gLog.error(new InternalServerError('oauth2Token middleware abstract password not implemented'));
                return Promise.reject(new InternalServerError());
            },
            ...
            refreshToken(pClient, pRefreshToken, pScope) {
                // @ts-ignore
                gLog.error(new InternalServerError('oauth2Token middleware abstract refreshToken not implemented'));
                return Promise.reject(new InternalServerError());
            },
            ...
            authorizationCode(pClient, pCode, pRedirectURI, pBody, pAuthInfo) {
                // @ts-ignore
                gLog.error(new InternalServerError('oauth2 middleware abstract authorizationCode not implemented'));
                return Promise.reject(new InternalServerError());
            },
            /**
             *
            ...
            userCredentials(pClient, pUsername, pPassword, pScope) {
                // @ts-ignore
                gLog.error(new InternalServerError('oauth2Token middleware abstract userCredentials not implemented'));
                return Promise.reject(new InternalServerError());
            }
        },
        pSource
    ...
    return endpointResponse((req, pResolve, pReject) => {
        //Pega token, conforme to grand_type definido do request
        getToken(req)
            .then(rResult => {
                pResolve({
