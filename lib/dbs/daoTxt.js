...
function daoTxt(pOptions) {
    ...
    const pvMetaSync = () => {
        return stat(wFile.filename).then(pStats => {
            ...
            return pvReadFile({ position: wFile.size, size: pStats.size - wFile.size }).then(xRows => {
                //Salva
                const xLastLine = xRows.length;
                for (let i = 0; i < xLastLine; i++) {
                    let xBytesInRow = Buffer.byteLength(xRows[i], this.options.encode);
                    //Não adiciona o byte da ultima linha
                    if (i + 1 !== xLastLine) {
                        xBytesInRow += Buffer.byteLength(this.options.rowDelimiter);
                    }
    ...
    const pvReadFile = pBlock => {
        ...
        return read(wFile.fd, Buffer.alloc(pBlock.size), 0, pBlock.size, pBlock.position).then(rResult => {
            const xArray = rResult.buffer.toString(this.options.encode).split(this.options.rowDelimiter);
