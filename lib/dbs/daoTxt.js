 ...
 *
 * @param {object} pOptions {encode: 'utf-8', rowDelimiter: '\n'}
 */
function daoTxt(pOptions) {
    this.options = { encode: 'utf-8', rowDelimiter: '\n', ...pOptions };
        ...
        return stat(wFile.filename).then(pStats => {
            ...
            return pvReadFile({ position: wFile.size, size: pStats.size - wFile.size }).then(xRows => {
                ...
                for (const xRow of xRows) {
                    pvAddMetaline(wFile.size, xRow.length + this.options.rowDelimiter.length);
                }
            });
        });
    };
    ...
    const pvAddMetaline = (pPosition, pSize) => {
        let xMetaline = { position: pPosition, size: pSize };
        wFile.metaline.push(xMetaline);
        wFile.size += xMetaline.size;
    };
    ...
    const pvGetBlock = pLines => {
        ...
        const xBlock = { size: xLineCurrent.size, position: xLineCurrent.position };
        //Retorna
        ...
        if (pLines > 0) {
            xBlock.size = xLineTarget.position + xLineTarget.size - xLineCurrent.position - xLineCurrent.size;
            xBlock.position = xLineCurrent.position + xLineCurrent.size;
        } else if (pLines < 0) {
            xBlock.size = xLineCurrent.position - xLineTarget.position;
            xBlock.position = xLineTarget.position;
        }
