...
const { deleteNull, objectCompliance } = require('investira.sdk').objects;
const { replaceAll } = require('investira.sdk').strings;
...
const sqls = {
    ...
    query: (pCN, pColumns, pSqlString, pValues = null, pClauses = {}) => {
        return new Promise((pResolve, pReject) => {
            try {
                let xDistinct = '';
                if (pClauses.hasOwnProperty('distinct')) {
    ...
    getSqlSelect: (pCN, pDaoMetadata, pConditions, pColumns = null, pClauses = {}) => {
        return pvSqlWhere(pCN, pDaoMetadata, pConditions).then(rWhere => {
            ...
            if (isEmpty(rWhere.sql)) {
                const xSize =
                    pClauses && ((pClauses.limit && pClauses.limit.size) || (isNumber(pClauses) ? pClauses : null));
                if (
                    (xSize === 1 && pColumns && pColumns.length !== 0) ||
                    (pConditions && !isEmpty(pConditions) && !rWhere.hasVariables)
                ) {
                    throw new QueryConditionsRequired(undefined, {
    ...
    setVariable: (pCN, pVariableName, pVariableValue) => {
        ...
        return pCN.sql(`SET ${pVariableName} = ?;`).bind(pVariableValue).execute();
    },
     ...
     *
     * @param {*} pErr
     * @param {object} pDetail Objeto com informação adicional
     * @returns
     */
                ...
                case 'number':
                    if (pOptions.toDB) {
                        ...
                        if (isNaN(xValueAux)) {
                            ...
                            xValueAux = null;
                        } else if (pControl.level === 1) {
                            //Pelomenos até a versão 8.0.17, o MYSQL arredonda valores em JS/Number.
...
const pvSqlUpdate = (pDaoMetadata, pData = {}) => {
    ...
    pvModelCompliance(
        ...
        { toDB: true }, //Considera que valores serão convertidos de JS para o formado do bando de dados
        {
            ...
            afterConvertValue: (pColumnValue, pControl) => {
                ...
                //Se coluna não for json
                if (pControl.columnPath === '') {
                    //Salva lista de valores
                    xValues.push(pColumnValue);
                    const xTmp = pControl.columnRoot + '=?';
                    //Inclui vírgula se já existir colunas a sem setaas
                    if (xSetColumns !== '') {
                        xSetColumns += `, `;
                    }
                    xSetColumns += xTmp;
                } else if (pControl.jsonColumnsForUpdate[pControl.columnRoot]) {
                    //Configura valor se coluna for json com partial update
                    let xTmp = '';
                    if (!xJsonColumns[pControl.columnRoot]) {
                        xTmp = pControl.jsonColumnsForUpdate[pControl.columnRoot];
                    } else {
                        xTmp = xJsonColumns[pControl.columnRoot];
                    }
                    //Seta placeholder com valor
                    if (
                        pColumnValue === null ||
                        pColumnValue === undefined ||
                        pControl.columnCondition.type === 'number'
                    ) {
                        //Seta como valor original
                        xTmp = xTmp.replace('"?"', pColumnValue);
                    } else {
                        //Subtitui as aspas duplas por aspas simples e seta como string
                        // xTmp = xTmp.replace('?', replaceAll(pColumnValue, '"', "'"));
                        xTmp = xTmp.replace('?', pColumnValue);
                    }
                    xJsonColumns[pControl.columnRoot] = xTmp;
                }
            }
        }
    );
    //Adiciona vírgula quando houver update de colunas normais e colunas json
    if (xSetColumns !== '' && Object.keys(xJsonColumns).length > 0) {
        xSetColumns += ', ';
    }
    // //Copia set das colunas json para set principal
    xSetColumns += Object.keys(xJsonColumns).reduce((pAcu, pKey, pIndex) => {
        if (pIndex > 0) {
            pAcu += ', ';
        }
        //Configura parcial update da coluna json
        pAcu += `${pKey}=JSON_MERGE_PATCH(IF(${pKey} IS NULL, JSON_OBJECT(),${pKey}),?)`;
        //Salva lista de valores
        xValues.push(xJsonColumns[pKey]);
        return pAcu;
    }, '');
    ...
    if (xSetColumns === '') {
        ...
        return {
            sql: `UPDATE ${pDaoMetadata.tableName} SET ${xSetColumns} `,
            values: xValues
        };
    }
};
...
const pvModelCompliance = (
    ...
    pControl = { level: 0, columnName: '', columnPath: '', columnRoot: '', columnFullName: '' }
) => {
    ...
    if (isObject(pData)) {
        ...
        for (const xColumnName in pData) {
            ...
            const xControl = {
                ...
                columnFullName: null,
                variableName: null,
                jsonColumnsForUpdate: pControl.jsonColumnsForUpdate || {}
            };
            ...
            if (pControl.level === 0 && pvGetTypeAttr(pModel[xColumnName]) === 'object') {
                ...
                if (pControl.level !== 0) {
                    if (isObject(pModel[xColumnName]) && !xControl.jsonColumnsForUpdate[xControl.columnRoot]) {
                        //Exclui atributos que não fazem parte do model
                        const xData = objectCompliance(pModel, pData);
                        //Substitui os valores pelo placeholder para ser utilizado no update
                        //e salva
                        xControl.jsonColumnsForUpdate[xControl.columnRoot] = JSON.stringify(xData, (_pKey, pValue) => {
                            if (!isObject(pValue)) {
                                if (
                                    ...
                                    return ` CAST(? AS DECIMAL(60,${(xDec[1] && xDec[1].length) || 0})) `;
                                } else {
                                    return `?`;
                                }
                            }
