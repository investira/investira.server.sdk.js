    ...
    rmDir: pDir => {
        return new Promise((pResolve, pReject) => {
            fs.access(pDir, pErr => {
                ...
                fs.readdir(pDir, (pErr, pFiles) => {
                    if (pErr) {
                        return pReject(pErr);
                    }
                    pFiles.map(async pFile => {
                        await files.remove(path.join(pDir, pFile));
                    });
                    fs.rmdir(pDir, rErr => {
                        if (rErr) {
                            return pReject(rErr);
        ...
        pFtpClient.on('ready', () => {
            pFtpClient.get(myURL.pathname, (pErr, pStream) => {
                ...
                pStream.once('close', pHadError => {
                    ...
                    if (!pHadError) {
                        // @ts-ignore
                        gLog.info(`Downloaded\t${myURL.pathname}`);
...
const pvDownloadHttp = (pUrl, pLocalPath, pProps = {}, pOnProgress = () => {}) => {
    const { params, headers, data, timeout, responseType = 'arraybuffer', method = 'GET' } = pProps;
    return httpRequests
        .request(
            method,
            {
        ...
        .then(rRes => {
            ...
            // @ts-ignore
            gLog.info(`Downloaded\t${pUrl} to ${xOutputFilename}`);
...
const pvReadFileXLS = (pFilename, pOptions = {}) => {
    ...
    return new Promise((pResolve, pReject) => {
        return fs.readFile(pFilename, (pErr, pData) => {
            ...
            try {
                const workbook = XLSX.read(pData, { type: 'buffer', ...pOptions });
                const sheet = workbook.Sheets[workbook.SheetNames[0]];
                //Converte para CSV
                const xRows = XLSX.utils.sheet_to_csv(sheet, { FS: '\t' }).split('\n');
                //Remove última linha
                xRows.pop();
                pResolve(
                    //Converte array de linhas contendo array de colunas
 ...
 *
 * @param {string} pFilename
 * @param {object} [pOptions={}] { strict: false }
 * @returns {Promise} Array das linhas contendo array das colunas lidas
 */
const pvReadFileXML = (pFilename, pOptions = {}) => {
    const parser = new xml2js.Parser(pOptions);
    return new Promise((pResolve, pReject) => {
        return fs.readFile(pFilename, (pErr, pData) => {
            if (pErr) {
                return pReject(pErr);
