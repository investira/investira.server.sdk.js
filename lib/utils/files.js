        ...
        return httpRequests
            ...
            .then(pResponse => {
                ...
                if (
                    ...
                    pResponse.request.res &&
                    pResponse.request.res.headers &&
                    pResponse.request.res.headers &&
                    pResponse.request.res.headers['content-disposition']
                ) {
                    let xFilenameRegex = /filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/;
                    let xMatches = xFilenameRegex.exec(pResponse.request.res.headers['content-disposition']);
                    ...
                    xOutputFilename = path.basename(pResponse.request.path);
                }
                xOutputFilename = path.join(pLocalPath, xOutputFilename);
                fs.writeFileSync(xOutputFilename, pResponse.data);
