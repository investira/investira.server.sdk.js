...
// const { exists, remove, rename } = require('investira.server.sdk').files;
...
const files = {
                        ...
                        try {
                            ...
                            pZipfile.openReadStream(pFile, (pErr, pReadStream) => {
                                ...
                                pReadStream.on('end', () => {
                                    ...
                                    xFiles.push(path.parse(pFile.fileName).base);
                                    //Busca próximo arquivo dentro do zip
                                    pZipfile.readEntry();
                                });
                                pReadStream.pipe(
                                    fs.createWriteStream(path.join(pTargetPath, path.parse(pFile.fileName).base))
                                );
                            });
                        } catch (rErr) {
                            return pReject(rErr);
     ...
     *
     * @param {string} pFilename
     * @param {object} [pOptions={}] {encode, rowDelimiter, columnDelimiter, columnEnclosure, removeSpacesFromFile}
    ...
    /**
     * Converte um conteúdo XML em objeto JavaScript.
     *
     * @param {string|Buffer} pConteudo Conteúdo XML em memória.
     * @param {object} [pOptions={}] { trim, explicitArray, stripPrefix, strict, normalizeTags, normalize, ignoreAttrs, explicitRoot }
     * @returns {Promise<object>} Objeto convertido a partir do XML informado
     */
    parseXML: (pConteudo, pOptions = {}) => {
        return pvParseXML(pConteudo, pOptions);
    },
    ...
    /**
     * Processa um arquivo removendo espaços iniciais de cada linha
     * @async
     * @function removeSpacesFromFile
     * @param {string} pArquivoOrigem - Caminho completo do arquivo XML de origem
     * @param {string|null} [pArquivoDestino=null] - Caminho completo do arquivo de destino (opcional)
     * @returns {Promise<Object>} Objeto com informações dos arquivos processados
     * @throws {Error} Quando ocorrem erros de validação ou processamento
     */
    removeSpacesFromFile: async (pArquivoOrigem, pArquivoDestino = null) => {
        // Validação do parâmetro de entrada
        if (!pArquivoOrigem) {
            throw new BasicMessageError(`Arquivo origem não definido.`);
        }

        // Verifica existência do arquivo de origem
        if (!files.exists(pArquivoOrigem)) {
            throw new BasicMessageError(`Arquivo origem inexistente.`);
        }

        // Configura caminho do arquivo de destino
        let xArquivoDestino = null;
        if (!pArquivoDestino) {
            // Cria nome temporário se nenhum destino for especificado
            xArquivoDestino = path.join(
                path.dirname(pArquivoOrigem),
                `${path.parse(pArquivoOrigem).name}_tmp${path.extname(pArquivoOrigem)}`
            );
        } else {
            // Usa o caminho especificado para o destino
            xArquivoDestino = path.join(path.dirname(pArquivoDestino), `${path.basename(pArquivoDestino)}`);
        }

        // Verifica existência do diretório de destino
        if (!files.exists(path.dirname(xArquivoDestino))) {
            throw new BasicMessageError(`Diretório de destino inexistente.`);
        }

        // Cria streams de leitura e escrita
        const xStreamLeitura = fs.createReadStream(pArquivoOrigem, { encoding: 'utf-8' });
        const xStreamEscrita = fs.createWriteStream(xArquivoDestino, { encoding: 'utf-8' });

        let xTamanhoOrigem = 0;
        let xTamanhoDestino = 0;
        let xBuffer = '';

        // Obtém tamanho do arquivo original
        try {
            const xStats = await fs.promises.stat(pArquivoOrigem);
            xTamanhoOrigem = xStats.size;
        } catch (xErro) {
            throw new BasicMessageError(`Erro ao obter tamanho do arquivo original: ${xErro.message}`);
        }

        return new Promise((pResolve, pReject) => {
            // Evento: dados lidos do arquivo de origem
            xStreamLeitura.on('data', xPedaco => {
                xBuffer += xPedaco;
                const xLinhas = xBuffer.split('\n');

                // Mantém a última linha incompleta no buffer
                xBuffer = xLinhas.pop() || '';

                // Processa cada linha removendo espaços iniciais
                for (const xLinha of xLinhas) {
                    const xLinhaProcessada = xLinha.replace(/^\s+/, '');
                    xStreamEscrita.write(xLinhaProcessada + '\n');
                    xTamanhoDestino += Buffer.byteLength(xLinhaProcessada + '\n', 'utf-8');
                }
            });

            // Evento: término da leitura do arquivo
            xStreamLeitura.on('end', () => {
                // Processa qualquer conteúdo restante no buffer
                if (xBuffer) {
                    const xLinhaProcessada = xBuffer.replace(/^\s+/, '');
                    xStreamEscrita.write(xLinhaProcessada);
                    xTamanhoDestino += Buffer.byteLength(xLinhaProcessada, 'utf-8');
                }

                xStreamEscrita.end();
            });

            // Evento: escrita concluída com sucesso
            xStreamEscrita.on('finish', () => {
                pResolve();
            });

            // Evento: erro na leitura
            xStreamLeitura.on('error', xErro => {
                xStreamEscrita.destroy();
                pReject(new BasicMessageError(`Erro na leitura do arquivo: ${xErro.message}`));
            });

            // Evento: erro na escrita
            xStreamEscrita.on('error', xErro => {
                pReject(new BasicMessageError(`Erro na escrita do arquivo: ${xErro.message}`));
            });
        })
            .then(() => {
                const xResult = {
                    arquivoOrigem: pArquivoOrigem,
                    tamanhoOrigem: xTamanhoOrigem,
                    arquivoDestino: xArquivoDestino,
                    tamanhoDestino: xTamanhoDestino
                };
                // Se nenhum destino foi especificado, substitui o arquivo original
                if (!pArquivoDestino) {
                    xResult.arquivoDestino = pArquivoOrigem;
                    return files
                        .remove(pArquivoOrigem, true)
                        .then(() => files.rename(xArquivoDestino, pArquivoOrigem))
                        .then(() => xResult);
                }
                return xResult;
            })
            .catch(rErr => {
                // Em caso de erro, remove arquivo temporário e rejeita a Promise
                return files
                    .remove(xArquivoDestino, true)
                    .then(() => Promise.reject(rErr))
                    .catch(() => Promise.reject(rErr)); // Garante que o erro original seja propagado
            });
    }
};
...
const pvDownloadHttp = (pUrl, pLocalPath, pProps = {}, pOnProgress = () => {}) => {
    ...
    if (
        ...
        String(headers['Content-Type']).toLocaleLowerCase().substring(0, 16) === 'application/json' &&
        method.toLocaleLowerCase() === 'post'
    ) {
        xData = data;
    } else {
        xData = querystring.stringify(data);
    }
    return httpRequests
        .request(
            method,
        ...
        return fs.readFile(pFilename, (pErr, pData) => {
            ...
            try {
                ...
                for (let xPageIndex = xPageIndexFirst; xPageIndex <= xPageIndexLast; xPageIndex++) {
                    ...
                    //Le configuração da range que será processado
                    let xRange = XLSX.utils.decode_range(xSheet['!ref']);
                    //Força todas as linhas
                    xRange.e.r = xSheet?.length || xRange.e.r;
                    xSheet['!ref'] = XLSX.utils.encode_range(xRange);
                    //Converte para CSV
 ...
 *
 * @param {string} pFilename
 * @param {object} [pOptions={}] { trim, explicitArray, stripPrefix, strict, removeSpacesFromFile }
 * @returns {Promise} Array das linhas contendo array das colunas lidas
 */
const pvReadFileXML = (pFilename, pOptions = {}) => {
    return new Promise((pResolve, pReject) => {
        const { removeSpacesFromFile = false } = pOptions;
        return Promise.resolve()
            .then(() => {
                //Remove espaços do arquivo para reduzir o risco de superar o limite de tamanho da string no parser.parseString
                if (removeSpacesFromFile) {
                    return files.removeSpacesFromFile(pFilename);
                }
                return Promise.resolve();
            })
            .then(() => {
                return fs.readFile(pFilename, (pErr, pData) => {
                    ...
                    return pvParseXML(pData, pOptions).then(pResolve).catch(pReject);
                });
            });
    });
};
...
/**
 * Converte conteúdo XML em objeto JavaScript.
 *
 * @param {string|Buffer} pConteudo Conteúdo XML a ser convertido.
 * @param {object} [pOptions={}] { trim, explicitArray, stripPrefix, strict, normalizeTags, normalize, ignoreAttrs, explicitRoot }
 * @returns {Promise<object>} Objeto convertido a partir do XML informado
 */
const pvParseXML = (pConteudo, pOptions = {}) => {
    return new Promise((pResolve, pReject) => {
        const {
            trim = true,
            strict = true,
        ...
        const xParser = new xml2js.Parser(xOptions);
        return xParser.parseString(pConteudo, (pErr, pResult) => {
            if (pErr) {
                return pReject(pErr);
            }
            //Força conversão para objeto literal, pois o xml2js retorna estrutura não derivada de Object.
