...
const sqls = {
            ...
            if (isEmpty(rWhere.sql)) {
                ...
                if (
                    ...
                    return Promise.reject(
                        new QueryConditionsRequired(undefined, {
    ...
    getSqlUpdate: (pCN, pDaoMetadata, pDataObject, pConditions, pClauses = {}) => {
        return pvSqlWhere(pCN, pDaoMetadata, pConditions).then(rWhere => {
            ...
            // @ts-ignore
            if (isEmpty(rWhere.sql) && !isNull(xClauses.limit)) {
                return Promise.reject(
    ...
    getSqlDelete: (pCN, pDaoMetadata, pConditions, pClauses = {}) => {
        return pvSqlWhere(pCN, pDaoMetadata, pConditions).then(rWhere => {
            if (isEmpty(rWhere.sql)) {
                return Promise.reject(
                    new QueryConditionsRequired('Conditions is required to filter the delete', { detail: rWhere.sql })
                );
            }
            let xSqlString = 'DELETE FROM ' + pDaoMetadata.tableName;
            xSqlString += rWhere.sql + pvGetClauses(pClauses);
    ...
    executeSql: (pCN, pRows, pMeta) => {
        ...
        return new Promise(async (pResolve, pReject) => {
            ...
            let xErr = null;
            let xDetail = {
                rawStatement: pCN.getSQL(),
                bindValues: pCN.bindValues
            };
