...
const COMPLEX_OPERATORS = ['MATCH', 'NOTMATCH', 'IN', 'NOTIN', ...COMPLEX_OPERATORS_BOOLEAN];
...
async function pvSqlWhere(pCN, pDaoMetadata, pConditions = {}, pOptions = {}) {
            ...
            afterConvertValue: (pColumnValue, pControl) => {
                ...
                if (isNull(pColumnValue)) {
                    ...
                    if (pControl.columnCondition.operator === 'IN' || pControl.columnCondition.operator === 'NOT IN') {
                        ...
                        xNewCondition = `${xNewCondition}`;
                    } else if (BINARY_OPERATORS.includes(pControl.columnCondition.operator)) {
                        //-------------------------------
                        // BINARY
                        //-------------------------------
    ...
    const xSqlConditionsComplex = pvToSQLWhereComplex(pConditions);
...
const pvToSQLWhereComplex = (pConditions, pComplex = false, pOperatorJoin = 'AND') => {
    ...
    for (let xKey in pConditions) {
        ...
        if (xKey === '$') {
            if (!COMPLEX_OPERATORS.includes(xConditions.toUpperCase())) {
                // @ts-ignore
                gLog.error(`Condition is not a valid operator: ${util.inspect(pConditions, undefined, 30)}`);
                return;
            }
            ...
            return pvToSQLWhereComplex(xConditions, true, pOperatorJoin);
        }
        ...
        if (COMPLEX_OPERATORS.includes(xOperatorKey.toUpperCase())) {
        ...
        } else if (COMPLEX_OPERATORS_BOOLEAN.includes(xOperatorKey.toUpperCase())) {
            ...
            const xResult = pvToSQLWhereComplex(xOperatorAttrs, pComplex, xOperatorKey);
