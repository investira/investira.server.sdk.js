const { isEmpty, isString } = require('investira.sdk').validators;
...
const sqls = {
    ...
    getSqlSelect: (
        ...
        let xSqlString = pDaoMetadata.tableName + pvGetWhere(pConditions);
        return sqls.query(
            pCN,
            pColumns || pDaoMetadata.columns,
            xSqlString,
            pvGetValues(pConditions),
            pLimit
        );
    },
    /**
    ...
    getSqlInsert: (pCN, pDaoMetadata, pDataObject) => {
        return new Promise((pResolve, pReject) => {
            ...
            pResolve(sqls.getSql(pCN, xSqlString, pvGetValues(xDataObject)));
        });
    },
    /**
     *
    ...
    getSqlUpdate: (
        ...
        return new Promise((pResolve, pReject) => {
            ...
            let xSqlString =
                ...
                pvGetWhere(pConditions) +
                pvGetLimit(pLimit);
            pResolve(
                sqls.getSql(pCN, xSqlString, [
                    ...pvGetValues(xDataObject),
                    ...pvGetValues(pConditions)
                ])
            );
        });
    ...
    getSqlDelete: (pCN, pDaoMetadata, pConditions, pLimit = null) => {
        return new Promise((pResolve, pReject) => {
            let xSqlString =
                ...
                pvGetWhere(pConditions) +
                pvGetLimit(pLimit);
            pResolve(sqls.getSql(pCN, xSqlString, pvGetValues(pConditions)));
        });
    },
    ...
    createMetadata: (pTableName, pTableModel) => {
        ...
        const xMetadata = {
            ...
            tableModel: dataModel(pTableModel), //Transforma tableModel em dataModel para controlar os valores das propriedades
