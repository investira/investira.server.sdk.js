...
const sqls = {
    ...
    executeSql: (pSql, pRows = null, pMeta = null) => {
        ...
        return new Promise(async (pResolve, pReject) => {
            ...
            console.log('executeSql', xDetail.rawStatment);
            //Efetua tentativa padrão e novas tentativas em caso de deadlock nos inserts
            while (xCount === 0 || (xInsert && xDeadLock && xCount < 30)) {
                xCount++;
                xDeadLock = false;
                xErr = null;
                console.log(xDetail.rawStatment, 1);
                await pSql
                    .execute(pRows, pMeta)
                    .then(rResult => {
                        console.log('executeSql', xDetail.rawStatment, 2);
                        return pResolve(rResult);
                    })
                    .catch(rErr => {
                        console.log('executeSql', xDetail.rawStatment, rErr);
