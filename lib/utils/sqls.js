...
const COMPLEX_OPERATORS = [...COMPLEX_OPERATORS_BASIC, ...COMPLEX_OPERATORS_BOOLEAN];
...
function pvSqlWhere(pCN, pDaoMetadata, pConditions = {}, pOptions = {}) {
    return pvSqlWhereRegular(pCN, pDaoMetadata, pConditions, pOptions).then(rWhereRegular => {
        ...
        return pvToSQLWhereComplex(pCN, pDaoMetadata, pConditions, pOptions).then(rWhereComplex => {
            if (rWhereRegular.sql === '' && rWhereComplex?.sql === '') {
                return { ...rWhereRegular, sql: '', values: [] };
            } else {
                let xSqlConditions = rWhereRegular.sql;
                //Combina condição normal com complexa
                if (rWhereRegular.sql !== '' && rWhereComplex?.sql !== '') {
                    xSqlConditions += ' AND ';
                }
                xSqlConditions += rWhereComplex?.sql || '';
                //Retorna
                return {
                    ...rWhereRegular,
                    sql: `WHERE ${xSqlConditions} `,
                    values: [...rWhereRegular.values, ...(rWhereComplex?.values || [])],
    ...
    for (let xKey in pConditions) {
        ...
        if (xComplexOperator === '$') {
            ...
            if (xOperator === '') {
                ...
                xResult = await pvSqlWhereRegular(pCN, pDaoMetadata, pConditions, pOptions);
            }
        }
