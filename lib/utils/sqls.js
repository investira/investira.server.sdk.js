...
const { isTrue, isEmpty, isArray, isNull, isNumber, isString, isDate, isObject, isUndefined } =
...
const { toTitleCase } = require('investira.sdk').strings;
...
const OPERATORS_BASIC = ['=', '<', '>', '>=', '<=', '!=', '<>'];
const OPERATORS_BINARY = [
    '&', //Contém todos os bits podendo ter outros '&?=?'
    '|', //Contém alguns dos bits '&?<>0'
    '!&', //Não contém nenhum dos bits '&?=0'
    '&=' //Contém somente os bits não podendo ter outros '^?=0'
];
...
const COMPLEX_OPERATORS_BASIC = ['MATCH', 'NOTMATCH', 'IN', 'NOTIN'];

const MATCH_RACKING_COLUMN = '__mr__';
const MATCH_OPERATORS = ['+', '-', '^', '*', '~', '>', '<', "'", '"'];

const MATCH_INVALIDATE_OPERATORS = [
    '++',
    '+*',
    '+-',
    '+@',
...
const sqls = {
    ...
    getSqlSelect: (pCN, pDaoMetadata, pConditions, pColumns = null, pClauses = {}) => {
        return pvSqlWhere(pCN, pDaoMetadata, pConditions, pClauses).then(rWhere => {
            ...
            const xColumns = pvGetQueryColumns(pDaoMetadata, pColumns);
            ...
            if (xColumns?.length === 0) {
                ...
                if (rWhere?.complex?.match_racking_column) {
                    xColumnsAsString += `,${rWhere.complex.match_racking_column} ${MATCH_RACKING_COLUMN}`;
                    rWhere.values = [...rWhere.complex.match_racking_values, ...rWhere.values];
     ...
     * 						  Os nomes dos atributos do object deve ser iguais aos respectivos nomes das colunas na tabela.
     * 						  ex: {client_id: 2, client_name: "teste"}
     *                       Coluna com função deve começar com '&' e conter array a string da função com placeholder ? como um valor e outro valor contendo outro array com a relação de valores.
     *                       '&pessoa_tipo_id': ['pessoa_tipo_id | ?', [1]]
     *                       '&pessoa_tipo_id': ['pessoa_tipo_id & ~?', [1]]
     * @param {object} pConditions Objecto com as condições
    ...
    createMetadata: (pTableName, pTableModel) => {
        try {
            ...
            for (const xKey in pTableModel) {
                if (!isObject(pTableModel[xKey])) {
                    throw new GeneralDataError(
                        `Atributo ${xKey} do model ${xMetadata.tableName} precisa ser um objeto válido`
                    );
                }
     ...
     * @returns
     */
    getSQLErrorClass: (pErr, _pDetail) => {
        if (pErr.info) {
    ...
    /**
     * Adiciona colunas de ordenação a cláusula sort
     * Se coluna informada já existir, não será adicionada
     *
     * @param {object} pClauses Atributo clauses do objeto de consulta
     * @param {Array<string>} pSortColumns Array com as colunas de ordenação
     * @returns {object} Cláusula com sort atualizado
     */
    addSortColumns: (pClauses, pSortColumns) => {
        if (!pSortColumns || !pSortColumns.length) {
            return pClauses;
        }
        const xClauses = { ...pClauses };
        xClauses.sort = String(pClauses.sort && pClauses.sort !== '' && pClauses.sort !== 'null' ? pClauses.sort : '');
        let xSortOriginal = String(xClauses.sort)
            .split(',')
            .filter(x => {
                return x !== '' && x !== null && x !== undefined;
            });
        for (const xSortColumn of pSortColumns) {
            if (!xSortOriginal.includes(xSortColumn)) {
                xSortOriginal.push(xSortColumn);
            }
        }
        xClauses.sort = xSortOriginal.join(',');
        return xClauses;
    }
};
    ...
    try {
        ...
        if (pColumnType && !isNull(xValue) && DATA_TYPES.includes(pColumnType.trim().toLowerCase())) {
            switch (pColumnType) {
                ...
                case 'tmp':
                    if (!pOptions.toDB) {
                        //Por ser uma coluna criada dinamicamente, tenta converter o conteúdo para objeto para caso seja json
                        //Se não conseguir, mantém o valor original
                        //O try/catch é para tratar o erro do parse para json, já que não há outra forma de testar se o valor é json
                        try {
                            const xValueTmp = JSON.parse(xValue);
                            // Confirma que o resultado é objeto ou array (não string, number, etc)
                            xValue = xValueTmp;
                        } catch {}
                    }
                ...
                case 'object':
                    if (pOptions.toDB) {
                        //Quando a atualização é na coluna inteira, é necessário fazer o stringfy pois o mysql faz a conversão para objeto
                        if (pControl.level === 1) {
                            if (isObject(xValue)) {
                            ...
                            //Quando é um objeto dentro de um atributo, utiliza o objeto sem conversão para string
                        } else if (!isObject(xValue)) {
                            try {
                                xValue = JSON.parse(xValue);
                            } catch {
                                // @ts-ignore
                                gLog.error(`Sqls: Not a valid object ${xValue}`);
                                xValue = null;
                            }
                        }
                    } else {
                        if (!isObject(xValue)) {
                            try {
                                let xValueTmp = JSON.parse(xValue);
                                xValue = xValueTmp;
                            } catch (rErr) {
                                // @ts-ignore
                                gLog.error(`Sqls: Not a valid object ${xValue}`);
                                xValue = { description: 'Sqls: Not a valid object' };
                            }
                        }
                    }
                    break;
                ...
                case 'array':
                    if (pOptions.toDB) {
                        ...
                        if (pControl.level == 1 || !pOptions.isInsert) {
                            xValue = JSON.stringify(xValue);
                        }
                    } else {
                        xValue = xValue;
...
function pvSqlInsert(pDaoMetadata, pData = {}, pOptions = {}) {
    ...
    pvModelCompliance(
        ...
        { ...pOptions, toDB: true, isInsert: true }, //Considera que valores serão convertidos de JS para o formado do bando de dados
        {
            ...
            afterConvertRoot: (pCoumnValue, pControl) => {
                ...
                if (pControl?.columnCondition?.type === 'array' || pControl?.columnCondition?.type === 'object') {
                    //Converte string de volta para número
                    xPlaceHolders += `CAST(? AS JSON)`;
                } else {
                    xPlaceHolders += '?';
                }
    ...
    if (xColumns === '') {
        ...
        return {
            sql:
                `INSERT ${xIgnore}` + `INTO ${pDaoMetadata.tableName}` + `(${xColumns}) ` + `VALUES (${xPlaceHolders})`,
            values: xValues
        };
    }
}
    ...
    pvModelCompliance(
        ...
        { ...pOptions, toDB: true }, //Considera que valores serão convertidos de JS para o formado do bando de dados
        {
            ...
            afterConvertValue: (pColumnValue, pControl) => {
                ...
                let xPlaceHolder = '?';
                //Coluna com função. Array contendo ['texto da função e place holder ? com para os valores',[array com valores]]
                if (pControl?.columnNameOriginal.startsWith('&')) {
                    if (pControl?.columnCondition?.value?.length === 2) {
                        xColumns += `(${pControl.columnCondition.value[0]})`;
                        xValues.push(...pControl.columnCondition.value[1]);
                    } else {
                        throw new GeneralDataError(`Incomplete instruction using function as value`);
                    }
                } else {
                    if (pControl?.columnCondition?.type === 'number' && !isEmpty(pControl.columnCondition.value)) {
                    ...
                    if (pControl?.columnCondition?.type === 'array' || pControl?.columnCondition?.type === 'object') {
                        //Converte string de volta para número
                        xPlaceHolder = ` CAST(? AS JSON) `;
                    }
                    //Se for partial json update
                    if (pControl.columnPath !== '') {
                        // xColumns += `JSON_SET(IFNULL(${pControl.columnRoot},JSON_OBJECT()),'$${pControl.columnPath}',?)`;
                        xColumns +=
                            '`JSON_MERGE_PATCH`(IF(' +
                            pControl.columnRoot +
                            ' IS NULL, JSON_OBJECT(),' +
 ...
 * @returns {Promise}
 */
function pvSqlWhere(pCN, pDaoMetadata, pConditions = {}, pOptions = {}) {
    //Configura as condições complexas se existirem
    return pvToSQLWhereComplex(pCN, pDaoMetadata, pConditions, pOptions).then(rWhereComplex => {
        if (rWhereComplex?.sql === '') {
            return { ...rWhereComplex, sql: '', values: [] };
        } else {
            let xSqlConditions = rWhereComplex?.sql || '';
            //Retorna
            return {
                ...rWhereComplex,
                sql: `WHERE ${xSqlConditions} `,
                values: [...(rWhereComplex?.values || [])],
                complex: rWhereComplex
            };
        }
    });
}
 ...
 * @returns {Promise}
 */
async function pvSqlWhereRegular(pCN, pDaoMetadata, pConditions = {}, pOptions = {}) {
    ...
    let xSqlConditions = '';
    let xVariables = {};
    //Função que será chamada para verificar se o DataModel é compatível com o TableModel
    pvModelCompliance(
        pDaoMetadata.tableModel,
        pConditions,
            ...
            afterConvertValue: (pColumnValue, pControl) => {
                ...
                if (isNull(pColumnValue)) {
                    ...
                    if (pControl.columnCondition.operator === 'IN' || pControl.columnCondition.operator === 'NOT IN') {
                        ...
                        xNewCondition = `${xNewCondition}`;
                    } else if (OPERATORS_BINARY.includes(pControl.columnCondition.operator)) {
                        //-------------------------------
                        // BINARY
                        //-------------------------------
                        //Define nome da coluna
                        xNewCondition += pControl.columnFullName;
                        //Define valor de comparação
                        xValues.push(pColumnValue);
                        if (pControl.columnCondition.operator === '&') {
                            //Contém todos os bits podendo ter outros
                            xValues.push(pColumnValue);
                            xNewCondition += '&?=?';
                        } else if (pControl.columnCondition.operator === '|') {
                            //Contém alguns dos bits
                            xNewCondition += '&?<>0';
                        } else if (pControl.columnCondition.operator === '!&') {
                            //Não contém nehum dos bits
                            xNewCondition += '&?=0';
                        } else if (pControl.columnCondition.operator === '&=') {
                            //Contém somente os bits não podendo ter outros
                            xNewCondition += '^?=0';
                        }
                    } else {
                        //Outros Operadores
                ...
                if (xNewCondition && xNewCondition !== '') {
...
const pvToSQLWhereComplex = async (pCN, pDaoMetadata, pConditions, pOptions, pOperatorJoin = 'AND') => {
    let xResultConditions = [];
    let xResultMatchRackingColumn = [];
    let xResultMatchRackingValues = [];
    let xResultSearchColumns = [];
    let xResultValues = [];
    // let xMatchRackingValues = [];
    //Loop por todos atributos
    for (let xKey in pConditions) {
        //Ignora atributos não complexos
        let xResult = null;
        let xComplexOperator = xKey.substring(0, 1);
        let xOperator = xKey.substring(1, 20).toLocaleUpperCase();
        const xConditions = pConditions[xKey];
        if (xComplexOperator === '$') {
            if (COMPLEX_OPERATORS_BOOLEAN.includes(xOperator)) {
                if (!isArray(xConditions) || !xConditions.length) {
                    // @ts-ignore
                    gLog.error(
                        `Condition must have an array with the conditions to be used with the operator '${xOperator}': ${util.inspect(
                            pConditions,
                            undefined,
                            30
                        )}`
                ...
                xResult = await pvToSQLWhereComplex(pCN, pDaoMetadata, xConditions, pOptions, xOperator);
            } else {
                if (COMPLEX_OPERATORS_BASIC.includes(xOperator)) {
                    if (!isObject(xConditions)) {
                        // @ts-ignore
                        gLog.error(
                            `Condition must be as object with 'columns' and 'value' atributes.'${xOperator}': ${util.inspect(
                                xConditions,
                                undefined,
                                30
                            )}`
                    ...
                    if (!xConditions.hasOwnProperty('value') || !xConditions.hasOwnProperty('columns')) {
                        // @ts-ignore
                        gLog.error(
                            `Condition must have a 'columns' and 'value' atributes.'${xOperator}': ${util.inspect(
                                xConditions,
                                undefined,
                                30
                            )}`
                    ...
                    xConditions.value = pvRemoveMatchInvalideOperators(xConditions.value);
                    //Adiciona valor a set pesquisado
                    xResultValues.push(xConditions.value);
                    let xNOT = '';
                    if (xOperator.substring(0, 3) === 'NOT') {
                        xNOT = 'NOT ';
                        xOperator = xOperator.substring(3, 10);
                    }
                    if (xOperator.indexOf('MATCH') > -1) {
                        //Exclui '+' em excesso
                        xConditions.value = xConditions.value.replace(/\b\w+\+\++(?=\s|$)/g, match =>
                            match.replace(/\++$/, '+')
                        );
                        //Palavras a serem pesquisadas, excluindo as inválidas e os operadores boleanos
                        const xWordsOnlyArray = pvMatchWords(xConditions.value);
                        //Se não houver palavras a serem pesquisadas
                        if (!xWordsOnlyArray.length) {
                            continue;
                        }
                        let xMode = '';
                        if (xConditions?.natural) {
                            xMode = 'IN NATURAL LANGUAGE MODE';
                            if (xConditions?.query_expansion) {
                        ...
                        //String MATCH
                        const xSql = `${xOperator}(${xConditions.columns}) AGAINST (? ${xMode})`;
                        xResultConditions.push(`${xNOT}${xSql}`);
                        //Colunas a serem pesquisadas
                        const xColumnsWithIFNULL = String(xConditions.columns)
                            .split(',')
                        ...
                        xMatchRankColumn = `${xSql} * (${xConditions.value.length} / (${xMatchRankColumn}))`;
                        xResultMatchRackingColumn.push(`${xMatchRankColumn}`);
                        xResultMatchRackingValues.push(xConditions.value);
                        xResultSearchColumns.push(xConditions.columns);
                        continue;
                    } else if (xOperator.indexOf('IN') > -1) {
                        xResultConditions.push(`(?) ${xNOT}${xOperator} (${xConditions.columns})`);
                        continue;
                    }
                }
            }
            ...
            if (xOperator === '') {
                xResult = await pvToSQLWhereComplex(pCN, pDaoMetadata, xConditions, pOptions, pOperatorJoin);
            } else {
                const xRegularCondtion = {};
                xRegularCondtion[xKey] = xConditions;
                xResult = await pvSqlWhereRegular(pCN, pDaoMetadata, xRegularCondtion, pOptions);
            }
        }
        ...
        if (xResult.sql) {
            ...
            if (xResult?.match_racking_column) {
                xResultMatchRackingColumn.push(xResult?.match_racking_column);
                xResultMatchRackingValues.push(...(xResult?.match_racking_values || []));
    ...
    return {
        ...
        match_racking_values: xResultMatchRackingValues,
...
const pvModelCompliance = (
    ...
    if (isObject(pData)) {
        ...
        for (const xColumnName in pData) {
            let xModelColumnName = xColumnName;
            ...
            const xControl = {
                ...
                columnFullName: null,
                columnNameOriginal: xColumnName.trim(),
                variableName: null
            };
            ...
            if (xColumnName.startsWith('@')) {
                ...
                //Coluna onde o valor é uma função
            } else if (xColumnName.startsWith('&')) {
                xModelColumnName = xColumnName.substring(1, xColumnName.length).trim();
                xControl.columnName = '`' + xModelColumnName + '`';
            }
            //Configura nome da coluna raiz
            if (pControl.level === 0) {
                xControl.columnRoot = xControl.columnName;
            } else {
                xControl.columnPath += '.' + xModelColumnName;
                //Configura nome da coluna caso seja utilizado no update
                xControl.columnPathForUpdate += 'JSON_OBJECT("' + xModelColumnName + '",';
            }
            ...
            const xType = pvGetTypeAttr(pModel[xModelColumnName]);
            //Ignora compliance. Aceita objeto sem tratamento dos seus atribuitos.
            if (
                xType === 'object' ||
                (pControl.level === 0 && (xType === 'tmp' || xModelColumnName === MATCH_RACKING_COLUMN))
            ) {
                //Recupera objeto convertido (valor final)
                xResult = pvModelComplianceConvertValue(
                    pModel[xModelColumnName],
                    pData[xColumnName],
                    pOptions,
                    pListeners,
                    xControl
                );
            } else if (pModel.hasOwnProperty(xModelColumnName)) {
        ...
        if (isArray(pData)) {
            ...
            if (pData.length > xTypeIndex) {
                ...
                if (!DATA_TYPES.includes(xColumn.type)) {
                    // @ts-ignore
                    throw new GeneralDataError(
                        `pvGetColumnConditionDetails:: Invalid data type "${util.inspect(xColumn)}"`
                    );
                }
            }
        }
 ...
 *
 * @param {object} pDaoMetadata Array origem
 ...
 */
const pvGetQueryColumns = (pDaoMetadata, pColumns) => {
    //Cria lista de colunas a partir do model ou das colunas informadas
    const xColumns = pColumns || pDaoMetadata?.columns || [];
    const xQueryColumns = [];
    for (const xKey of xColumns) {
        const xIsFormula = Array.isArray(xKey);
        //Se for um array, considera que o primeiro item é o nome do campo
        const xWord = xIsFormula ? xKey[0] : xKey;
        if (xWord === undefined) {
            throw new GeneralDataError(`Column undefined ${pColumns}`);
        }
        //Exclui colunas de variável de ambiente do banco de dados
        if (!xWord.startsWith('@')) {
            //Se coluna estiver no model ou coluna for uma formula ou MATCH_RACKING_COLUMN
            if (xWord === MATCH_RACKING_COLUMN || pDaoMetadata.columns.includes(xWord) || xIsFormula) {
                xQueryColumns.push(xKey);
                //Força a inclusão da coluna no model para evitar que seja desprezado do complience
                if (xIsFormula && !pDaoMetadata.tableModel[xWord]) {
                    pDaoMetadata.tableModel[xWord] = { type: 'tmp' };
                }
            }
        }
    }
...
/**
 * Remove caracteres inválidos utilizados no fulltext search
 *
 * @param {String} pString
 * @return {String}
 */
const pvRemoveMatchInvalideOperators = pString => {
    if (!pString) {
        return pString;
    }
    let xOk = false;
    //Loop até não existis mas itens, assumindo que pode haver erro contendo mais de dois caracteres como '***';
    while (!xOk) {
        xOk = true;
        for (const xOperator of MATCH_INVALIDATE_OPERATORS) {
            if (pString.indexOf(xOperator) > -1) {
                // @ts-ignore
                pString = pString.replaceAll(xOperator, xOperator.substring(0, 1));
                xOk = false;
            }
        }
    }
