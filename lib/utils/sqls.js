...
const OPERATORS = ['=', '<', '>', '>=', '<=', '!=', '&', '|', 'IN', 'NOT IN', 'NOT LIKE', 'LIKE', 'BETWEEN'];
                ...
                case 'number':
                    if (pToDB) {
                        ...
                        if (!isNull(xValueTmp)) {
                            ...
                            //FIXME: Aguardando acerto pela Oracle
    ...
    pvModelCompliance(
        ...
        { isConditions: true },
        {
            ...
            afterConvertValue: (pColumnValue, pControl) => {
                ...
                //Parenteses de seguranção INICIO
                xColumns += '(';
                //Nome da coluna
                xColumns += pControl.columnRoot;
                if (pControl.columnPath !== '') {
                ...
                if (isNull(pColumnValue)) {
                    ...
                    if (pControl.columnCondition.operator === 'IN' || pControl.columnCondition.operator === 'NOT IN') {
                        ...
                        //Inclui opetaador da condição
                        xColumns += ` ${pControl.columnCondition.operator} `;
                        if (
                            pControl.columnCondition.operator === 'IN' ||
                            pControl.columnCondition.operator === 'NOT IN'
                        ) {
                            xColumns += sqls.getPlaceHolders(xValuesArray.length - xNullCount);
                        }
...
const pvModelCompliance = (
    ...
    pOptions = {},
    pListeners = {},
    pControl = { level: 0, columnName: '', columnPath: '', columnRoot: '', columnPathForUpdate: '' }
) => {
