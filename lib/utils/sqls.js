    ...
    executeSql: (pCN, pSql) => {
        ...
        return new Promise((pResolve, pReject) => {
            ...
            while (xCount === 0 || (xDeadLock && xCount < 30)) {
                ...
                xDeadLock = false;
                xErr = null;
                pCN.execute(pSql.sqlString, pSql.bindValues)
                    .then(rResult => {
                        xResult = rResult[0];
                        pResolve(xResult);
                    })
...
const toSQLWhereComplex = (pConditions, pComplex = false, pOperatorJoin = 'AND') => {
    ...
    for (let xKey in pConditions) {
        ...
        xOperatorAttrs.value = String(xOperatorAttrs?.value || '');
        xOperatorKey = xOperatorKey.toUpperCase();
        if (COMPLEX_OPERATORS.includes(xOperatorKey)) {
            if (!xOperatorAttrs?.columns || !xOperatorAttrs.hasOwnProperty('value') || xOperatorAttrs?.value === '') {
                // @ts-ignore
                gLog.error(
            ...
            //Palavras a serem pesquisadas, excluindo as inválidas
            const xMacthWordsArray = pvMatchWords(xOperatorAttrs?.value || '');
            xResultValues.push(xMacthWordsArray.join(' '));
            //Se não houver palavras a serem pesquisadas
            if (!xMacthWordsArray.length) {
                continue;
            }
            ...
            if (xOperatorKey.indexOf('MATCH') > -1) {
                ...
                let xMatchRackingColumn = xMacthWordsArray.reduce((pResultado, pPalavra) => {
                    // if (pPalavra.substring(0, 1) !== (xNOT ? '-' : '+')) {
                    //     return pResultado;
                    // }
                    if (pResultado) {
                        pResultado += '+';
                    }
                    pPalavra = pPalavra.replace(/[+-<>~*\"\']/g, '');
                    pResultado += `LOCATE('${pPalavra}', CONCAT(${xColumnsWithIFNULL}))`;
                    return pResultado;
                }, '');
                xMatchRackingColumn = `${xConditionString} * (LENGTH('${xOperatorAttrs.value}') / (${xMatchRackingColumn}))`;
                // if (xMatchRackingColumn.length > 0) {
                // } else {
                //     xMatchRackingColumn = xConditionString;
                // }
                xResultMatchRackingColumn.push(`${xMatchRackingColumn}`);
