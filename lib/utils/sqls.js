...
// @ts-ignore
const { objectCompliance } = require('investira.sdk').objects;
// @ts-ignore
...
const sqls = {
    ...
    getSqlSelect: (
        ...
        let xConditions = objectCompliance(pConditions, pDaoMetadata.columns);
        let xSqlString =
            '`' +
            pDaoMetadata.tableName.trim() +
            '` ' +
    ...
    getSqlInsert: (pCN, pDaoMetadata, pDataObject) => {
        return new Promise((pResolve, pReject) => {
            ...
            let xDataObject = objectCompliance(
                pDataObject,
    ...
    getSqlDelete: (pCN, pDaoMetadata, pConditions, pLimit = null) => {
        return new Promise((pResolve, pReject) => {
            //Exclui as propriedades contidas em pConditions cujos nomes NÃO façam parte de pDaoMetadata.columns
            let xConditions = objectCompliance(
                pConditions,
                pDaoMetadata.columns
            );
            if (isEmpty(xConditions)) {
                return pReject(
                    new QueryConditionsRequired(
                        'Conditions is required to filter the delete'
                    )
                );
            }
            let xSqlString =
    ...
    createMetadata: (pTableName, pTableModel) => {
        try {
            ...
            const xKeys = Object.keys(pTableModel);
            for (let xKey of xKeys) {
                //Salva nome das colunas editáveis
                if (
...
const pvGetWhere = pConditions => {
    ...
    let xWhere = '';
    for (let xColumnName of xColumns) {
        ...
        if (isNull(xColumnDetail.value)) {
            ...
            if (xColumnDetail.operator === 'IN') {
                ...
                if (isArray(xColumnDetail.value)) {
                    ...
                    xWhere += sqls.getPlaceHolders(
                        xColumnDetail.value.split(',').length
                    );
                }
...
const pvGetValuesFromDataObject = (pDaoMetadata, pDataObject) => {
    try {
        ...
        return Object.values(
            pvConvertValues(pDaoMetadata, pDataObject, true, true)
        );
    } catch (err) {
        // @ts-ignore
        gLog.error(err);
...
const pvGetValuesFromConditions = (pDaoMetadata, pConditions) => {
    try {
        //Retorna valores convertidos conforme definição do daometadata
        return Object.values(
            pvConvertValues(pDaoMetadata, pConditions, false, true)
        );
    } catch (err) {
        // @ts-ignore
        gLog.error(err);
        ...
        for (let xColumnName in pObject) {
            ...
            if (xColumnDetail.operator === 'IN') {
                ...
                for (const xItem of xValues) {
                    xItens.push(
                        pvConvertValue(xItem, xColumnDetail.type, pConvertToDB)
                    );
                }
                xValue = xItens;
            } else {
