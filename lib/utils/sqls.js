...
const pvModelCompliance = (
    ...
    if (isObject(pData)) {
        ...
        for (const xColumnName in pData) {
            ...
            if (
                pControl.level === 0 &&
                (pvGetTypeAttr(pModel[xModelColumnName]) === 'object' || xModelColumnName === MATCH_RACKING_COLUMN)
            ) {
    ...
    for (const xKey of xColumns) {
        ...
        if (!xWord.startsWith('@')) {
            ...
            if (xWord === MATCH_RACKING_COLUMN || pDaoMetadata.columns.includes(xWord) || xIsFormula) {
                ...
                //Forma a inclusão da coluna no model para evitar que seja desprezado do complience
                if (xIsFormula && !pDaoMetadata.tableModel[xWord]) {
                    pDaoMetadata.tableModel[xWord] = {};
                }
            }
        }
    }
