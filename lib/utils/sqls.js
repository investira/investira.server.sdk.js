...
const sqls = {
    ...
    getSqlUpdate: (pCN, pDaoMetadata, pDataObject, pConditions, pLimit) => {
        return pvSqlWhere(pCN, pDaoMetadata, pConditions).then(rWhere => {
            ...
            if (isEmpty(rWhere.sql) && isUndefined(pLimit)) {
                throw new QueryConditionsRequired(
                    `limit=null must be informed to update all rows on query: ${xSql.sql}`,
                    {
                        details: xSql.sql
                    }
                );
     ...
     * @param {function} [pRows] Função chamada a cada leitura de registro contendo os dados lidos.
     * @param {function} [pMeta] Função chamada a cada leitura de registro contendo
     * 								  o metadado da coluna conforme definido diretamente no banco.
     * @returns {Promise}
     */
    executeSql: (pSql, pRows, pMeta) => {
        if (isNull(pSql)) {
...
/**
 * Retorna clause Group by, Having, Limit e Order By
 *
...
const pvGetClauses = pClauses => {
    ...
    //Having
    let xHaving = '';
    if (!isEmpty(pClauses.having)) {
        xHaving = `HAVING ${pClauses.group} `;
    }
    //LIMITS
    let xLimit = null;
    let xClauses = {};
    ...
    if (xClauses.hasOwnProperty('limit')) {
    ...
    return xGroupBy + xHaving + xOrderBy + pvGetLimit(xLimit);
};
