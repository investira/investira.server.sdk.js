...
const pvGetQueryColumns = (pModel, pColumns) => {
    ...
    for (const xKey of xColumns) {
        const xFormula = Array.isArray(xKey);
        //Se for um array, considera que o primeiro item é o nome do campo
        const xWord = xFormula ? xKey[0] : xKey;
        if (xWord === undefined) {
        ...
        if (!xWord.startsWith('@')) {
            ...
            if (xWord === MATCH_RACKING_COLUMN || pModel.includes(xWord) || xFormula) {
