...
// const ATTRIBUTES = ['autoIncrement', 'generated', 'type'];
...
const pvConvertValue = (pColumnValue, pColumnType, pToDB = true) => {
    try {
        ...
        if (!isNull(xValue) && pColumnType && DATA_TYPES.includes(pColumnType.trim().toLowerCase())) {
            switch (pColumnType) {
                case 'string':
                    if (pToDB) {
                        if (!isString(xValue)) {
...
const pvGetColumnConditionDetails = (pColumnName, pColumnCondition, pTableModel = {}) => {
    try {
        ...
        if (!xColumn.hasOwnProperty('type')) {
            if (pTableModel.hasOwnProperty(xColumn.name) && pTableModel[xColumn.name].hasOwnProperty('type')) {
