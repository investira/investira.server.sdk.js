...
const sqls = {
    ...
    getSqlInsert: (pCN, pDaoMetadata, pDataObject) => {
        return new Promise((pResolve, pReject) => {
            const xSql = pvSqlInsert(pDaoMetadata, pDataObject);
            if (isEmpty(xSql.sql)) {
 ...
 * @returns {object}
 */
const pvSqlInsert = (pDaoMetadata, pData = {}) => {
    const xValues = [];
    let xColumns = '';
    pvModelCompliance(
        ...
        { toDB: true },
        {
            afterConvertRoot: (pData, pControl) => {
                if (isObject(pData)) {
                    xValues.push(pvConvertValue(pData, 'object', true));
                } else {
                    xValues.push(pData);
                }
                if (xColumns !== '') {
...
const pvSqlUpdate = (pDaoMetadata, pData = {}) => {
    ...
    pvModelCompliance(
        ...
        { isConditions: true },
        {
            afterConvertValue: (pData, pControl) => {
                if (pData === null) {
                    return;
                }
                if (xColumns !== '') {
...
const pvModelComplianceConvertValue = (pModel, pData, pOptions, pListeners, pControl) => {
    try {
        let xValue;
        pControl.columnCondition = pvGetColumnConditionDetails(pData, pModel, pOptions.isConditions);
        ...
        if (isArray(pControl.columnCondition.value)) {
            //Converte valores dentro do array
            if (pOptions.isConditions) {
                xValue = [];
                ...
                // @ts-ignore
                gLog.error(new GeneralDataError(`pvModelComplianceConvertValue: Array not allowed`));
                throw new GeneralDataError();
            }
        } else {
            //Efetua convertão do valor
 ...
 * @returns
 */
const pvGetColumnConditionDetails = (pData, pModel = {}, pIsConditions = false) => {
    try {
        let xColumn = {
            operator: '=',
            value: pData,
        ...
        if (pIsConditions) {
            ...
            if (isArray(pData)) {
                ...
                //ATRIBUTO:TYPE
                if (pData.length > 2) {
                    xColumn.type = pData[2].trim().toLowerCase();
