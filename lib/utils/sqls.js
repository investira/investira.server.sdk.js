 ...
 * @param {object} pColumnLevel
 * * @param {object} pOptions
 * @returns {array}
 */
const pvConvertValue = (pColumnValue, pColumnType, pColumnLevel, pOptions) => {
    try {
                    ...
                    if (pOptions.toDB) {
                        ...
                        if (isNaN(xValueTmp)) {
                            ...
                            if (xValue !== '' && xValue !== 'null') {
                                // @ts-ignore
                                throw new GeneralDataError(`Value not numeric [${xValue}]`);
                            }
                            xValueTmp = null;
                        } else if (pColumnLevel === 1 || !pOptions.isInsert) {
        ...
        { toDB: true, isInsert: true }, //Considera que valores serão convertidos de JS para o formado do bando de dados
        {
            ...
            afterConvertRoot: (pCoumnValue, pControl) => {
                ...
                if (isObject(pCoumnValue)) {
                    xValues.push(pvConvertValue(pCoumnValue, 'object', pControl.level, { toDB: true, isInsert: true }));
                } else {
