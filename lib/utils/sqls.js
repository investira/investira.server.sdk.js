const { isEmpty, isArray, isNull } = require('investira.sdk').validators;
...
const sqls = {
     ...
     * @returns
     */
    query: (pCN, pColumns, pSqlString, pValues = null, pLimit = null) => {
    ...
    getSqlDelete: (pCN, pDaoMetadata, pConditions, pLimit = null) => {
        return new Promise((pResolve, pReject) => {
            ...
            if (isEmpty(xConditions)) {
                return pReject(
                    new InvalidData('No Conditions To Filter The Delete')
                );
            }
            let xSqlString =
                'DELETE FROM ' +
...
const pvGetWhere = pConditions => {
    ...
    xColumns.forEach(xColumnName => {
        ...
        if (isNull(xColumnDetail.value)) {
            ...
            //Operador padrão
            xWhere += xColumnName + xColumnDetail.operator + '?';
        }
    });
    return 'WHERE ' + xWhere + ' ';
};
...
const pvGetColumnDetails = (pColumnName, pColumnValue, pTableModel = {}) => {
    try {
        ...
        if (isArray(pColumnValue)) {
            ...
            if (!OPERATORS.includes(pColumnValue[0].trim())) {
                gLog.error(
                    new GeneralDataError(
                        `Invalida operator "${pColumnValue[0]}" for column "${
                            xColumn.name
                        }"`
                    )
                );
            ...
            xColumn.operator = pColumnValue[0];
            //ATRIBUTO:VALUE
            ...
            if (pColumnValue.length > 2) {
                if (
                    !DATA_TYPES.includes(pColumnValue[2].trim().toLowerCase())
                ) {
                    gLog.error(
                        new GeneralDataError(
                            `Invalid data type "${
                                pColumnValue[2]
                            }" for column "${xColumn.name}"`
                        )
                    );
                    throw new GeneralDataError();
                }
                xColumn.type = pColumnValue[2];
            }
        }
