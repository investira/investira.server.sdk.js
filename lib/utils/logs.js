...
const { stringify } = require('investira.sdk').strings;
...
const errorObject = format((pInfo, _pArg) => {
    ...
    if (stack && level === 'error' && (showStack || !isBasicMessage)) {
        ...
        // @ts-ignore
        xResult.message = (message && message.message) || message || description;
    }
    if (!xResult.message) {
        xResult.message = "Couldn't find message to log";
    } else if (isObject(xResult.message)) {
        //Transforma objeto em json"
        // @ts-ignore
        xResult.message = stringify(xResult.message);
    }
...
const isEnabled = () => {
    try {
        ...
        if (gRC) {
            ...
            if (gRC.hasOwnProperty('hasContext') && gRC.hasContext()) {
                // @ts-ignore
                const xDisableLog = gRC.get('disable_log') || false;
                return !xDisableLog;
            }
        }
    } catch (err) { }
...
const pvCreateIgnoreFormat = pValidator => {
    return format(pInfo => {
        if (!pValidator(pInfo)) {
            return false;
        }
...
const consoleFormat = format.combine(
    pvCreateIgnoreFormat(isEnabled)(),
    errorObject(),
    colorize(),
    timestamp(),
    prettyPrint(),
 ...
 * Formatação para o console
 */
const fileFormat = format.combine(pvCreateIgnoreFormat(isEnabled)(), errorObject(), timestamp(), json());
...
function logs(pOptions = {}) {
    const xLogger = createLogger({
        ...
        exitOnError: false,
        format: format.combine(pvCreateIgnoreFormat(isEnabled)(), format.errors({ stack: true }), format.json()) //format.metadata()
    });
